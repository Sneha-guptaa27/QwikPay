...
const detectHeaderIndex = (matrix) => {
  // Look in first 30 rows for something that has "date" AND ("description"/"narration"/"details"/"particular")
...
function parseXlsx(buffer, mapping) {
  ...
  // 1) choose the most "dataful" sheet (not the first cover sheet)
  const sheetName = pickDenseSheet(wb);
  const ws = wb.Sheets[sheetName];

  // 2) read as a matrix (no header inference yet)
  const matrix = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });

  // 3) find header row (or honor explicit mapping.headerRowIndex)
  let headerRowIndex = typeof mapping?.headerRowIndex === "number"
  ...
  // 4) detect column indices by header text (very tolerant)
  const dateIdx    = mapping?.dateIdx    ?? idxOf([/date/i, /txn\s*date/i, /value\s*date/i, /posting/i]);
  const descIdx    = mapping?.descIdx    ?? idxOf([/description/i, /narration/i, /details/i, /particular/i, /reference/i]);
